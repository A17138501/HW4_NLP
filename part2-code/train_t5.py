...
def eval_epoch(args, model, dev_loader, gt_sql_pth, model_sql_path, gt_record_path, model_record_path):
    ...
    Dev 上的 evaluation：
      ...
      * 用 model.generate 生成 SQL
      * 用 save_queries_and_records 写出 .sql/.pkl
      * 用 compute_metrics 算 SQL EM / Record EM / Record F1 / error_rate
    '''
    model.eval()
    criterion = nn.CrossEntropyLoss()
    total_loss = 0.0
    ...
    # 3）保存预测 SQL + records
    # 注意：这里的调用顺序是 (queries, sql_path, record_path)
    ...
    # 🔑 compute_metrics 返回的 error_rate 可能是 float / str / list[str|float]
    try:
        if isinstance(error_rate, list):
            if len(error_rate) > 0:
                vals = [float(x) for x in error_rate]
                error_rate_val = sum(vals) / len(vals)
            else:
                error_rate_val = 0.0
        else:
            error_rate_val = float(error_rate)
    except (TypeError, ValueError):
        # 出现奇怪类型时兜底
        error_rate_val = 0.0

    return avg_loss, record_f1, record_em, sql_em, error_rate_val
...
def main():
    ...
    # Evaluate best checkpoint on dev and generate final dev/test outputs
